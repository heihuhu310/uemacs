...
char *gtfun(char *fname)
{
	...
	switch (fnum) {
	case UFADD:
		return eitoa(atoi(arg1) + atoi(arg2));
	case UFSUB:
		return eitoa(atoi(arg1) - atoi(arg2));
	case UFTIMES:
		return eitoa(atoi(arg1) * atoi(arg2));
	...
	case UFGTKEY:
		...
		return result;
	case UFRND:
		return eitoa((ernd() % abs(atoi(arg1))) + 1);
	case UFABS:
		return eitoa(abs(atoi(arg1)));
	case UFSINDEX:
		return eitoa(sindex(arg1, arg2));
	case UFENV:
#if	ENVFUNC
		tsp = getenv(arg1);
		return tsp == NULL ? "" : tsp;
...
char *gtenv(char *vname)
{
	...
	switch (vnum) {
	case EVFILLCOL:
		return eitoa(fillcol);
	case EVPAGELEN:
		return eitoa(term.t_nrow + 1);
	case EVCURCOL:
		return eitoa(getccol(FALSE));
	case EVCURLINE:
		return eitoa(getcline());
	case EVRAM:
		return eitoa((int) (envram / 1024l));
	case EVFLICKER:
		return ltos(flickcode);
	case EVCURWIDTH:
		return eitoa(term.t_ncol);
	case EVCBUFNAME:
	...
	case EVASAVE:
		return eitoa(gasave);
	case EVACOUNT:
		return eitoa(gacount);
	case EVLASTKEY:
		return eitoa(lastkey);
	case EVCURCHAR:
		return (curwp->w_dotp->l_used ==
			curwp->w_doto ? eitoa('\n') :
			eitoa(lgetc(curwp->w_dotp, curwp->w_doto)));
	case EVDISCMD:
		return ltos(discmd);
	case EVVERSION:
		return VERSION;
	...
	case EVDISINP:
		return ltos(disinp);
	case EVWLINE:
		return eitoa(curwp->w_ntrows);
	case EVCWLINE:
		return eitoa(getwpos());
	case EVTARGET:
		saveflag = lastflag;
		return eitoa(curgoal);
	case EVSEARCH:
		return pat;
	case EVREPLACE:
		return rpat;
	...
	case EVKILL:
		return getkill();
	case EVCMODE:
		return eitoa(curbp->b_mode);
	case EVGMODE:
		return eitoa(gmode);
	case EVTPAUSE:
		return eitoa(term.t_pause);
	case EVPENDING:
#if	TYPEAH
...
#endif
	case EVLWIDTH:
		return eitoa(llength(curwp->w_dotp));
	case EVLINE:
		return getctext();
	case EVGFLAGS:
		return eitoa(gflags);
	case EVRVAL:
		return eitoa(rval);
	case EVTAB:
		return eitoa(tabmask + 1);
	case EVOVERLAP:
		return eitoa(overlap);
	case EVSCROLLCOUNT:
		return eitoa(scrollcount);
#if SCROLLCODE
	case EVSCROLL:
